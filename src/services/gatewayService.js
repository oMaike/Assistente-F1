  ...
  if (req.method === "GET" && url.pathname === "/api/architecture") {
    sendJson(res, 200, {
      ...
      services: [
        ...
        { name: "Knowledge Base Service", port: config.services.knowledge.port },
        { name: "External API Service", port: config.services.externalApi.port },
        { name: "Explanation Service", port: config.services.explanation.port },
      ],
    });
    return;
  }
